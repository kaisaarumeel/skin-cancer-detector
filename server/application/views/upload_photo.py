...
class UploadPhoto(View):
    def post(self, request):
        ...
        if file_extension not in ['.jpeg', '.jpg', '.png']:
            return JsonResponse({"err": "Invalid file type. Only JPEG, JPG, and PNG are allowed."}, status=400)
        ...
        if not localization or not lesion_type:
            return JsonResponse({"err": "Missing localization or lesion_type"}, status=400)
        ...
        try:
            ...
            return JsonResponse({'err': f'Error processing image: {str(e)}'}, status=500)
