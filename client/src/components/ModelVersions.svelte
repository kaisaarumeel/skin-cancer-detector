...
<p class="mt-6 text-sm font-medium text-tertiary">Switch the current Model Version:</p>
<ul class="mt-1 w-full space-y-2 text-gray-700 max-h-80 overflow-y-auto">
    ...
    <li>
        ...
        on:keydown={(e) => e.key === 'Enter' && setCurrentVersion(version)}
        >
        <div>
            <span class="text-tertiary">{version.name}</span>
            <span class="text-tertiary text-xs ml-2">{version.dateAdded}</span>
        </div>
