    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if not metadata_path.exists():
                return JsonResponse({"err": "Metadata CSV not found in zip file"}, status=400)
            ...
            if not images_path.exists():
                return JsonResponse({"err": "Images folder not found in zip file"}, status=400)
            ...
            for file in os.listdir(images_path):
                ...
                if file_extension in valid_extensions:
                    image_id = os.path.splitext(file)[0]  # get image name without the extension
                    image_files[image_id] = images_path / file # image name is key, path is value
            ...
            for index, row in df.iterrows():
                ...
                if not image_path or not image_path.exists():
                    print(f"Warning: image {row['image_id']} not found or invalid and will be skipped")
                    continue
            ...
            return JsonResponse({"message": "Training data uploaded successfully", "training_data_size": training_data_size})
