...
class ModelTests(TestCase):
    def setUp(self):
        ...
        for i in range(5):
            ...
            if i == 0:
                self.active_model = model # save a reference to the active model for test data
    ...
    def test_get_all_models_success(self):
        ...
        response_data = json.loads(response.content)
        self.assertIn("models", response_data) # check whether key 'models' exists in response data

    def test_swap_model_success(self):
        """Test successfully swapping active model"""
        self.client.force_login(self.test_user_admin)
        new_model = Model.objects.filter(status="archived").first() # get the first archived model
        ...
        self.assertIn(f"Active model swapped to model {new_model.model_id} version {new_model.version}", response_data["message"]) # check whether the response contains the correct success message
        
        new_model.refresh_from_db()
        self.assertEqual(new_model.status, "active") # check that the new model is now active
    ...
    def test_swap_model_not_found(self):
        ...
        self.assertEqual(response_data["error"], "Model not found.") # check whether the response contains the correct error message
    ...
    @patch('application.models.Model.save', side_effect=Exception("Forced internal server error"))
    def test_swap_model_internal_server_error(self, mock_save):
        ...
        response_data = json.loads(response.content)
        self.assertIn("error", response_data)
        self.assertIn("Failed to swap models", response_data["error"]) # check whether the response contains the correct error message